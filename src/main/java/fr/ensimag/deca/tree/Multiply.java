...
public class Multiply extends AbstractOpArith {
    ...
    public void codeGenExp(DecacCompiler compiler, int reg) {
        ...
        if(this.getLeftOperand() instanceof FloatLiteral ){
            ...
            compiler.addInstruction(new MUL(new ImmediateFloat(fl.getValue()),compiler.processor.getReg(reg)));
            ...
            this.getRightOperand().codeGenExp(compiler, reg);
            compiler.addInstruction(new MUL(new ImmediateInteger(in.getValue()),compiler.processor.getReg(reg)));
            if((!compiler.getCompilerOptions().getNoCheck()) && (getLeftOperand().getType().isFloat() || getRightOperand().getType().isFloat())){
                compiler.processor.addBovToArithLabel(compiler);
            }
            ...
            if(reg < compiler.processor.Rmax) {
                ...
                compiler.addInstruction(new MUL(compiler.processor.getReg(reg + 1),compiler.processor.getReg(reg)));
                if((!compiler.getCompilerOptions().getNoCheck()) && (getLeftOperand().getType().isFloat() || getRightOperand().getType().isFloat())){
                    compiler.processor.addBovToArithLabel(compiler);
                }
                ...
                compiler.addInstruction(new MUL(compiler.processor.getR0(),compiler.processor.getReg(reg)));
                if((!compiler.getCompilerOptions().getNoCheck()) && (getLeftOperand().getType().isFloat() || getRightOperand().getType().isFloat())){
                    compiler.processor.addBovToArithLabel(compiler);
                }
            }
